...
- **Columns** — attributes that describe each record
...
By joining the `CUSTOMER` and `TRANSACTION` tables on `CustomerID`, you can produce entirely new result sets — such as a customer statement consolidating all transactions within a given period — with a single query.

```sql
-- Example: Retrieve all transactions for a customer in Q1 2024
SELECT
    c.CompanyName,
    t.TransactionDate,
    t.TransactionAmount,
...
## RDBMS vs. Flat Files (e.g., Spreadsheets)

Relational databases build on the organizational principles of flat files (rows and columns), but that is where the similarity ends.
...
- Export and import operations can run **while the database is live**
- Cloud-based RDBMS instances perform **continuous mirroring**, meaning data loss on failure can be measured in **seconds or less**
...
Cloud relational databases offer access to **limitless compute and storage** capabilities and are gaining widespread adoption.
...
RDBMS is well-suited for OLTP workloads because it:
- Accommodates a **large number of concurrent users**
- Supports **insert, update, and delete** of small data volumes at high rates
- Handles **frequent queries** with **fast response times**
...
```mermaid
graph TD
    A[RDBMS] --> B[Tables with Rows & Columns]
    A --> C[Relationships via Foreign Keys]
    A --> D[SQL as Query Language]
    A --> E[ACID Compliance]
    A --> F[Use Cases]
    F --> G[OLTP]
    F --> H[Data Warehousing / OLAP]
    F --> I[IoT]
    A --> J[Limitations]
    J --> K[No support for unstructured data]
    J --> L[Migration complexity]
    J --> M[Field length constraints]
```
...
- Popular platforms span open-source (MySQL, PostgreSQL) and commercial (Oracle, SQL Server, IBM DB2), with cloud-based equivalents available from all major providers.
